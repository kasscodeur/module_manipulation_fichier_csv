...
from module import (
    load_csv,
    first_preprocess_data,
    preprocess_data,
    most_common_job,
    calculate_subscription_rate,
    filter_by_age
)
...
def user_greating() -> None:
    ...
    client_data = first_preprocess_data(client_data_csv)
    ...
    while True:
        ...
        print(
            "5 : Aperçu du fichier avec valeurs catégorielles converties en numériques"
        )
        ...
        if user_input == 0:
            ...
            try:
                ...
                print("Nom de colonne inexsitante\n")
            ...
            print(f"\nliste des colonnes {client_data.nom_colonnes}")
            try:
                nom_colonne = input("entrez le nom de la colonne  : ").strip().lower()
                client_data.get_colonne(nom_colonne).stats()
            except:
                print("Nom de colonne inexistante\n")
        ...
        while True:
            other_operation = input(
                "Souhaitez vous réaliser une autre opération ? (Oui/Non) : "
            ).title()
